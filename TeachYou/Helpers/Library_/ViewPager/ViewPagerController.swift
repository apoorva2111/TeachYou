...
public class ViewPagerController:UIViewController {
    ...
    fileprivate func setupTabContainerView() {
        ...
        tabContainer.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
        tabContainer.roundCornerWithShadow(shadowColor: .black, radius: 0, borderWidth: 0, borderColor: .clear, shadowOpacity: 0.1 , shadowRadius: 4, shadowOffsetWidth: 0, shadowOffsetHeight: 4)
    ...
    fileprivate func setupTabs() {
        ...
        if options.fitAllTabsInView {
            ...
            for (index,eachTab) in tabsList.enumerated() {
                ...
                tabView.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
                tabView.tag = index
            ...
            if options.isEachTabEvenlyDistributed {
                ...
                for (index,eachTabView) in tabsViewList.enumerated() {
                    ...
                    eachTabView.updateFrame(atIndex: index, withWidth: self.view.frame.width/self.tabsList.count, options: options)
                    let seperator = UIView()
                    seperator.backgroundColor = .clear
                    if index == 0{
                        seperator.frame = CGRect(x: self.view.frame.width/self.tabsList.count, y: 0, width: 1, height: options.tabViewHeight)
                    }
                    else if index == 1{
                        seperator.frame = CGRect(x: (self.view.frame.width/self.tabsList.count) * 2, y: 0, width: 1, height: options.tabViewHeight)
                    }
                    tabContainer.addSubview(eachTabView)
                    tabContainer.addSubview(seperator)
                }
                
                totalWidth = maxWidth * CGFloat(tabsViewList.count)
            }
            
            tabContainer.contentSize = CGSize(width: self.view.frame.width, height: options.tabViewHeight)
        }
    }
    ...
    fileprivate func createPageViewController() {
        ...
        pageViewController = UIPageViewController(transitionStyle: options.viewPagerTransitionStyle, navigationOrientation: UIPageViewController.NavigationOrientation.horizontal
            , options: nil)
