...
class Utilities {
    ...
    class func initDOBPicker(delegate:DatePickerViewDelegate,mode:Int = 0, isDOB: Bool?){
        ...
        (mode == 0) ? ((isDOB == true) ? (dobPicker?.pickerTitle.text = Validation.kDOBPlaceholder) : (dobPicker?.pickerTitle.text = Validation.kAlAnonBirthdayPlaceholder)) : (dobPicker?.pickerTitle.text = Validation.kSelectTimePlaceholder)
    ...
    class func showDialogue(text:String,delg:DialogueViewDelegate,vc:UIViewController){
        (vc as? BaseViewController)?.hideNavigationBar()
