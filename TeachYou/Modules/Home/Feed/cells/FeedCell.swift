...
class FeedCell: UITableViewCell {
    ...
    @IBOutlet weak var lblReactCount: UILabel!
    ...
    @IBOutlet weak var viewDeletePostOutlet: UIButton!
    @IBOutlet weak var lblSharePostDiscription: UILabel!

    //    var feelings : [Feelings]?
    var isOtherUserPost = false
    ...
    func setdata(post : Posts){
        ...
        if post.reactions_total_count ?? 0 > 0 {
            self.lblReactCount.text = "\(post.reactions_total_count ?? 0)"
        } else {
            self.lblReactCount.isHidden = true
        }
        
        if post.comments ?? 0 > 0 {
            self.lblCommentCount.text = "\(post.comments ?? 0)"
        } else {
            self.lblCommentCount.isHidden = true
        }
        
        if post.shares ?? 0 > 0 {
            self.lblShareCount.text = "\(post.shares ?? 0)"
        } else {
            self.lblShareCount.isHidden = true
        }
        ...
        if(imageViewPost != nil){
            if (post.photos?.count == 0) || (post.photos?.count == nil) || (post.photos?.count ?? 0 < 0) {
    ...
    func setMyPostdata(post : Posts?, originArr:NSDictionary){
        self.postModel = post
...
//            self.labelDescription.text = ""
    ...
    @IBAction func shareBtnTapped(_ sender: UIButton) {
        delegate?.shareAction(pid: self.postId)
    }
